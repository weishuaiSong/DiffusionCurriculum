...
class TemplateGenerator:
    ...
    def _create_meta_templates_prompts(self,counts: int) -> List[str]:
        ...
        """
        prompt_template = f"""Please generate {counts} different instruction meta templates structures for Visual Question Answering (VQA) tasks that include multiple choice options.     
...
"placeholder_options": {
 ...
 """
    ...
    async def generate_meta_templates(self, counts: int) -> List[Dict[str, Any]]:
        ...
        try:
            parsed_results = [JSONParser(result) for result in results]
